...
9. dropoff_before_pickup -- WARN at 0.1%. The same condition as Silver's dropoff_before_pickup_flag; rates above the threshold indicate systemic timestamp corruption rather than isolated row-level source anomalies (D29).
...
12. passenger_count_gt_8 -- WARN at 0.1%. Rates above the threshold are treated as a material departure from the source baseline and a possible source-quality, schema, or column-mapping problem; the worst month so far was 0.0156% (D29).
13. vendor_id_domain
