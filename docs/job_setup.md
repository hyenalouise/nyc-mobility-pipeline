...
| Schedule | Weekly, Friday 11:30 `America/New_York` (23:30 Manila on EDT, 22:30 on EST). Paused in `dev`, running in `prod`. Weekly because the source is monthly, so a daily run would find nothing new most days (D27) |
