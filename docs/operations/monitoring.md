...
- **Execution** — did the Control gate run: status, failures, duration, recency. Read from `pipeline_runs` and the Pipeline Execution dashboard. `pipeline_runs` is written only by `90_validate_control.sql`, so it reflects the Control gate, not the whole job: a run that fails later at Silver, Gold or Analytics still shows `SUCCESS` here, and a run stopped by a source gate never reaches the Control gate and writes no row. For whole-job status, use the job's run history in the Jobs UI.
...
| Fails | `gate_status` per layer, "Recent Control Gate Runs," "Stuck Runs Detected" (the `no_stuck_runs` check) | Any gate `FAILED`; for stuck runs specifically, any run still `STARTED` more than 6 hours after it began (`stuck_after_hours`, `etl/01_control/90_validate_control.sql`) | This is the gate design working as intended, not an anomaly. Every task depending on the failed gate is skipped by task dependency (`job-setup.md`'s Tasks table). Find the failed gate, then look at its own `FAIL` rows in the Data Quality dashboard for why it blocked. A stuck run usually means a crash or a hung job rather than something genuinely in progress, and it blocks every later run: `no_stuck_runs` is WARN at 0.0%, so the Control gate fails until the row is cleared. Confirm in the Jobs UI that the run is really dead, then mark it failed: ``UPDATE `ftw-week-08`.`01-control`.pipeline_runs SET status = 'FAILED', completed_at = current_timestamp() WHERE run_id = '<run_id>' AND status = 'STARTED'`` |
| Duration | "Latest Control Gate Duration," "Average Duration (Last 20 Runs)," "Control Gate Duration Trend" | None set. `job-setup.md` already states no duration threshold is configured. | No automatic action. A person watching the trend chart is how a slow warehouse, a serverless cold start, or a growing dataset gets noticed today. |
| Recency | "Hours Since Last Control Gate Run," "Date of Last Control Gate Run" | None automated. The job runs weekly (Friday 11:30 `America/New_York`), so meaningfully more than 168 hours since the last run, with no known schedule change, is worth a look. | Check the job's schedule is still enabled and not paused in that target (`dev` is paused by design; `prod` should not be). |
