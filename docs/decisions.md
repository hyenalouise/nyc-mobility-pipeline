...
| D27 | Run the job weekly, Monday 06:00 New York time, and let the target decide whether the schedule is paused | Approved | The job runs without someone starting it, and freshness has an interval to be measured against | **Amended (09/25/2026):** moved from Monday 06:00 to Friday 11:30 `America/New_York`, so the run lands at the end of the team's week in Manila (23:30 while New York is on EDT, 22:30 on EST). Still weekly, for the same reason as above: the source is monthly, so a daily run would find nothing new most days. The timezone stays New York, the reporting timezone.
